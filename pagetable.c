...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// get the pte for the victim page
		pgtbl_entry_t* victim_pte = coremap[frame].pte;
		// get offest in the swap file
		off_t victim_swap_offset = victim_pte->swap_off;
		// make sure page isn't already in swap
		if ((victim_pte->frame & PG_ONSWAP) == 0) {
			// if the page is dirty, when can write to swap
			if (victim_pte->frame & PG_DIRTY) {
				// swap the page out, get returned offset
				int swapped_page_offset = swap_pageout(victim_pte->frame >> PAGE_SHIFT,victim_swap_offset);
				// if it didnt work
				if (swapped_page_offset == INVALID_SWAP) {
					// error msg
				...
				// successfully written
					...
					// save the offset in the swap file
					victim_pte->swap_off = swapped_page_offset;
					// increment the dirty evicted counter
					evict_dirty_count++;
				}
			// clean
			} else {
				// increment the clean evicted counter
				evict_clean_count++;
			}
			// page is not valid, clear validity in pte
			victim_pte->frame &= ~PG_VALID;
			// page is now onswap, set onswap in pte
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if(p->frame & PG_VALID){
		// page valid increment hit_count
		hit_count++;
	} 
	else if(p->frame & PG_ONSWAP) {
		// page invalid on swap
		miss_count++;
		// get frame from coremap
		int frame = allocate_frame(p);
		// swap page and memory
		int swap = swap_pagein(frame, p->swap_off);
		// check if swapping caused errors
		if(swap != 0){
			perror("swap page in error");
			exit(0);
		}
		// store physical address upper bits of p-frame with PG_ONSWAP bit
		p->frame = frame << PAGE_SHIFT;
		p->frame = p->frame & ~PG_ONSWAP;
	}
	else{
		// page invalid not on swap
		miss_count++;
		// get frame from coremap
		int frame = allocate_frame(p);
		//save vaddr in page
		init_frame(frame, vaddr);
		// store physical address upper bits of p-frame with PG_DIRTY bit
		p->frame = frame << PAGE_SHIFT;
